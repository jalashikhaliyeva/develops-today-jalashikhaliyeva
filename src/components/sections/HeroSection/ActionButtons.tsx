...
export function ActionButtons({
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-6 justify-center items-center animate-[fadeInUp_1s_ease-out_1s_both]">
      <button
        onClick={() => window.open("https://jala-shikhaliyeva-portfolio.vercel.app/", "_blank")}
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
        className="group relative inline-flex items-center px-8 py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-2xl font-normal text-lg transition-all duration-300 transform hover:scale-105 hover:shadow-2xl hover:shadow-purple-500/25 overflow-hidden cursor-pointer"
      >
        {/* Button glow effect */}
      ...
      <button
        ...
        className="group inline-flex items-center px-8 py-4 bg-white/80 backdrop-blur-sm border border-gray-200 text-gray-700 rounded-2xl font-normal text-lg transition-all duration-300 transform hover:scale-105 hover:bg-white hover:shadow-xl hover:border-purple-300 cursor-pointer"
      >
